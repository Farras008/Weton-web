...
function database(): PDO
{
    ...
    foreach (['DB_HOST', 'DB_NAME', 'DB_USER'] as $key) {
        if (app_config($key, '') === '') throw new RuntimeException('Database belum dikonfigurasi (' . $key . '). Buat config/local.php dari config/local.example.php atau set environment variable terkait.');
    }
